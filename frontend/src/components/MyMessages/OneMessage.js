...
export const OneMessage = ({ direction }) => {
  const near = useContext(NearContext);

  return (
    <div className={`flex flex-row ${direction === "in" ? "justify-start" : "justify-end"}`}>
      <div className="messages text-sm text-white grid grid-flow-row gap-2">
        <div className="flex items-center flex-row-reverse group">
          <p className={`px-6 py-3 max-w-xs lg:max-w-md
            ${direction === "in" ? "bg-gray-800 rounded-r-full text-gray-200" : "bg-blue-700 rounded-l-full"}
            rounded-t-full`}>
            Hey! How are you?
          </p>
